...
VALUES ('*',1,extract(epoch FROM now())::int,31337,extract(epoch FROM now())::int);

INSERT into channels (name,flags,channel_ts,registered_ts,last_updated)
VALUES ('#coder-com',1,extract(epoch FROM now())::int,31337,extract(epoch FROM now())::int);

INSERT into users (user_name,password,language_id,last_updated,created_ts) 
VALUES ('Admin','xEDi1V791f7bddc526de7e3b0602d0b2993ce21d',1,extract(epoch FROM now())::int,extract(epoch FROM now())::int);

INSERT into users_lastseen (user_id,last_seen,last_updated)
VALUES (1,extract(epoch FROM now())::int,extract(epoch FROM now())::int);

INSERT into levels (channel_id,user_id,access,last_updated)
VALUES (1,1,1000,extract(epoch FROM now())::int);

INSERT into levels (channel_id,user_id,access,last_updated)
VALUES (2,1,500,extract(epoch FROM now())::int);
